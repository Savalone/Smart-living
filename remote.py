...
def send_ir_signal(signal):
    ...
    time.sleep(1)  # Esperar un poco para asegurarse de que el comando se envíe
...
def callback_query(call):
    if call.data == 'Power':
        tv_power_signal = "ir universal tv Power"
        response = send_ir_signal(tv_power_signal)
        bot.send_message(call.message.chat.id, "Ha sido presionado el botón Power. Respuesta del Flipper Zero: " + response)
    elif call.data == 'Mute':
